import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";

interface Accessoires {
  id: number;
  nom: string;
  prix: number;
  image?: string;
}

interface ItemPanier {
  produit: string;
  imageProduit: string;
  quantite: number;
  accessoiresParDefaut: Accessoires[];
  accessoiresOptionnels: Accessoires[];
  totalPrice: number;
}

const Panier = () => {
  const [panier, setPanier] = useState<ItemPanier[]>([]);

  useEffect(() => {
    const localPanier = localStorage.getItem("panier");
    if (localPanier) {
      const panierData: ItemPanier[] = JSON.parse(localPanier).map((item: any) => ({
        ...item,
        accessoiresOptionnels: item.accessoiresOptionnels || item.accessoires || [],
        accessoiresParDefaut: item.accessoiresParDefaut || [],
      }));
      setPanier(panierData);
    }
  }, []);

  const syncLocalStorage = (majPanier: ItemPanier[]) => {
    localStorage.setItem("panier", JSON.stringify(majPanier));
    window.dispatchEvent(new Event("panierMAJ"));
  };

  const calculTotal = (): number => {
    return panier.reduce((sum, item) => sum + item.totalPrice, 0);
  };

  const generatePDF = () => {
    const doc = new jsPDF({ unit: "mm", format: "a4" });
    
    // En-tête avec fond et texte
    doc.setFillColor(242, 242, 242);
    doc.rect(0, 0, 210, 40, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text("BON DE COMMANDE", 200, 15, { align: "right" });
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.text("Usine Renault de Sandouville", 200, 23, { align: "right" });
    
    // Logo (placeholder base64 court)
    const logoPlaceholder = "data:image/png;base64,";
    doc.addImage(logoPlaceholder, "PNG", 14, 8, 25, 18);
    
    // Informations de commande
    const currentDate = new Date().toLocaleDateString();
    doc.text(`Date : ${currentDate}`, 14, 50);
    doc.text(`Commande n°: ${Math.floor(Math.random() * 100000)}`, 14, 55);
    doc.setLineWidth(0.5);
    doc.setDrawColor(200, 200, 200);
    doc.line(14, 60, 196, 60);
    
    // Préparation des données pour le tableau PDF
    autoTable(doc, {
      startY: 65,
      head: [["Produit", "Accessoires", "Quantité", "Sous-total"]],
      headStyles: {
        fillColor: [230, 230, 230],
        textColor: [0, 0, 0],
        fontSize: 10,
        fontStyle: "bold",
      },
      bodyStyles: {
        textColor: [0, 0, 0],
        fontSize: 9,
      },
      alternateRowStyles: {
        fillColor: [245, 245, 245],
      },
      body: panier.map((item) => {
        const allAccessories = [...item.accessoiresParDefaut, ...item.accessoiresOptionnels];
        const accessoriesText = allAccessories.length
          ? allAccessories.map((acc) => `${acc.nom} (${acc.prix} €)`).join(", ")
          : "Aucun";
        return [
          item.produit,
          accessoriesText,
          item.quantite.toString(),
          `${item.totalPrice.toFixed(2)} €`
        ];
      }),
    });
    
    const finalY = (doc as jsPDF & { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? 0;
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.text(`Total : ${calculTotal().toFixed(2)} €`, 14, finalY + 10);
    
    // Pied de page avec numéro de page
    const pageCount = doc.getNumberOfPages();
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i);
      doc.setFontSize(8);
      doc.setTextColor(100);
      doc.text(`Page ${i} / ${pageCount}`, 105, 290, { align: "center" });
    }
    
    doc.save("bon-de-commande.pdf");
  };

  const majQuantite = (index: number, newQuantite: number) => {
    if (newQuantite < 1) return;
    setPanier((prevPanier) => {
      const majPanier = [...prevPanier];
      const prixUnitaire = majPanier[index].totalPrice / majPanier[index].quantite;
      majPanier[index].quantite = newQuantite;
      majPanier[index].totalPrice = prixUnitaire * newQuantite;
      syncLocalStorage(majPanier);
      return majPanier;
    });
  };

  const retirerItem = (index: number) => {
    setPanier((prevPanier) => {
      const majPanier = prevPanier.filter((_, i) => i !== index);
      syncLocalStorage(majPanier);
      return majPanier;
    });
  };

  return (
    <div className="container mx-auto p-4">
      {panier.length === 0 ? (
        <div className="text-center mt-20">
          <p className="text-gray-600 mb-4">Votre panier est vide.</p>
          <Link
            to="/"
            className="inline-block bg-yellow-500 text-white px-6 py-2 rounded-lg shadow-md hover:bg-yellow-600 transition-colors duration-300"
          >
            Retour à la boutique
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Liste des articles du panier */}
          <div className="lg:col-span-2 flex flex-col gap-4">
            {panier.map((item, index) => (
              <div
                key={index}
                className="border border-gray-200 rounded-lg p-4 flex gap-4 items-center shadow-sm hover:shadow-md transition-shadow duration-300"
              >
                <div className="w-20 h-20 flex-shrink-0 relative">
                  <img
                    src={item.imageProduit}
                    alt={item.produit}
                    className="object-contain rounded-lg w-full h-full"
                  />
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-lg text-gray-800">{item.produit}</p>
                  <p className="text-sm text-gray-600 mt-1">
                    Accessoires :{" "}
                    {((item.accessoiresParDefaut || []).concat(item.accessoiresOptionnels || [])).length > 0
                      ? ((item.accessoiresParDefaut || []).concat(item.accessoiresOptionnels || []))
                          .map(acc => acc.nom ? `${acc.nom} (${acc.prix} €)` : "")
                          .join(", ")
                      : "Aucun"}
                  </p>
                  <div className="flex flex-wrap gap-2 mt-2 items-center text-sm">
                    <p className="bg-gray-100 px-2 py-1 rounded-md">
                      Quantité : <strong>{item.quantite}</strong>
                    </p>
                  </div>
                  <p className="text-xl font-bold text-gray-800 mt-2">
                    Total : {item.totalPrice.toFixed(2)}€
                  </p>
                </div>
                <div className="flex flex-col items-end gap-2">
                  <input 
                    type="number"
                    min="1"
                    value={item.quantite}
                    onChange={(e) =>
                      majQuantite(index, parseInt(e.target.value))
                    }
                    className="border border-gray-200 px-2 py-1 w-16 text-center rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors duration-200"
                  />
                  <button
                    onClick={() => retirerItem(index)}
                    className="text-red-500 hover:text-red-700 transition-colors text-sm"
                  >
                    Supprimer
                  </button>
                </div>
              </div>
            ))}
          </div>

          {/* Récapitulatif et génération du PDF */}
          <div className="lg:sticky lg:top-16 flex flex-col gap-4 h-fit self-start p-4 border border-gray-200 rounded-lg shadow-sm">
            <div>
              <h2 className="text-xl font-bold text-gray-800 mb-2">Récapitulatif</h2>
              <p className="text-sm text-gray-600">
                Vérifiez le total et générez votre PDF.
              </p>
            </div>
            <div className="border-t pt-4">
              <div className="flex items-center justify-between">
                <span className="text-gray-700 font-semibold">Total :</span>
                <span className="text-xl font-bold text-gray-800">
                  {calculTotal().toFixed(2)} €
                </span>
              </div>
            </div>
            <div className="mt-4">
              <button
                onClick={generatePDF}
                className="w-full bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-yellow-600 transition-colors duration-300 font-semibold cursor-pointer"
              >
                Générer le PDF
              </button>
            </div>
            <div className="text-center mt-2">
              <Link
                to="/"
                className="inline-block text-sm text-yellow-600 hover:text-yellow-700 transition-colors"
              >
                Continuer mes achats
              </Link>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default Panier;
