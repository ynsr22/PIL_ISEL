...
const Panier = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const panier: ItemPanier[] = JSON.parse(localStorage.getItem("panier") || "[]").map(item => ({
        ...item,
    ...
    const generatePDF = () => {
      ...
      autoTable(doc, {
        ...
        body: panier.map((item) => {
          // Fusionne les accessoires par défaut et optionnels
  ...
  // Mise à jour de la quantité d'un article et recalcule son sous-total
  const majQuantite = (index: number, newQuantite: number) => {
    ...
    setPanier((prevPanier) => {
      ...
      majPanier[index].totalPrice = prixUnitaire * newQuantite; // Mise à jour du total
  ...
  // Calcule le total du panier en sommant les sous-totaux de chaque article
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            {panier.map((item, index) => (
              <div
                key={index}
                className="border rounded-lg p-4 flex gap-4 items-center shadow-sm hover:shadow-md transition-shadow duration-300"
              >
              ...
              <div className="flex flex-col items-end gap-2">
                <input 
                  ...
                  className="border border-gray-300 px-2 py-1 w-16 text-center rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-colors duration-200"
                  />
                  <button
          ...
          <div className="lg:sticky lg:top-16 flex flex-col gap-4 h-fit self-start p-4 border rounded-lg shadow-sm">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Vérifier le total et générez votre PDF.
              </p>
            </div>
            ...
            <div className="mt-4">
              <button
                onClick={generatePDF}
                className="w-full bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-yellow-600 transition-colors duration-300 font-semibold"
              >
                Générer le PDF
              </button>
            </div>
